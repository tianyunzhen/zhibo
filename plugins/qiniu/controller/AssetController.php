...
class AssetController extends PluginBaseController
{
    ...
    function getUrl()
    {
        ...
        $previewUrl = $fileType == 'image' ? $qiniu->getPreviewUrl($file)
            : $qiniu->getFileDownloadUrl($file);
        $url        = $fileType == 'image' ? $qiniu->getImageUrl($file,
            'watermark') : $qiniu->getFileDownloadUrl($file);
    ...
    public function saveFile()
    {
        ...
        $encodedEntryURISrc  = \Qiniu\base64_urlSafeEncode($config['bucket']
            . ':' . $fileKey);
        $encodedEntryURIDest = \Qiniu\base64_urlSafeEncode($config['bucket']
            . ':' . $fileKey . ".{$suffix}");
