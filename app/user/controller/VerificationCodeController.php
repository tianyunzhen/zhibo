    ...
    public function send()
    {
        ...
        if (isset($data['type']) && $data['type'] == 'register') {
            if ($accountType == 'email') {
                $findUserCount = db('user')->where('user_email',
                    $data['username'])->count();
            } else {
                if ($accountType == 'mobile') {
                    $findUserCount = db('user')->where('mobile',
                        $data['username'])->count();
                }
            }
        ...
        if ($accountType == 'email') {
            ...
            $username = empty($user['user_nicename']) ? $user['user_login']
                : $user['user_nicename'];

            $message = htmlspecialchars_decode($emailTemplate['template']);
            $message = $this->view->display($message,
                ['code' => $code, 'username' => $username]);
            $subject = empty($emailTemplate['subject']) ? 'ThinkCMF验证码'
            ...
            if ($accountType == 'mobile') {
                ...
                $expireTime = empty($result['expire_time']) ? 0
                    : $result['expire_time'];
