...
class ShareController extends HomebaseController
{
    ...
    public function index()
    {
        ...
        $list = Db::name('live')->field("uid,title,city,stream,pull,thumb")
            ->where(["islive" => 1])->order("isrecommend desc,starttime desc")
            ->limit(0, 20)->select()->toArray();
    ...
    public function show()
    {
        ...
        $liveinfo = Db::name("live")
            ->field("uid,islive,stream,pull,isvideo,type,goodnum,wy_cid")
            ->where(['uid' => $roomnum, 'islive' => 1])->find();
        if (!$liveinfo) {
        ...
        if ($liveinfo['isvideo'] == 1) {
            ...
            if ($liveinfo['islive'] && $liveinfo['type'] == 0) {
                if ($configpri['cdn_switch'] == 5) {
                    ...
                    $hls = PrivateKeyA('http', $liveinfo['stream'] . '.m3u8',
                        0);
                }
        ...
        if ($uid) {
            ...
            $isexist = Db::name('user_attention')->where([
                'uid'   => $uid,
                'touid' => $liveinfo['uid'],
            ])->find();
            if ($isexist) {
    ...
    public function wxLogin()
    {
        ...
        $callback = get_upload_path('/wxshare/Share/wxLoginCallback?roomnum='
            . $roomnum); //回调地址
        //微信登录
        if (!session_id()) {
            session_start();
        ...
        $wxurl
            = "https://open.weixin.qq.com/connect/oauth2/authorize?appid={$AppID}&redirect_uri={$callback}&response_type=code&scope=snsapi_userinfo&state={$state}#wechat_redirect ";
    ...
    public function wxLoginCallback()
    {
        ...
        if ($code) {
            ...
            /* 刷新token 有效期为30天 */
            $url
                = "https://api.weixin.qq.com/sns/oauth2/refresh_token?appid={$AppID}&grant_type=refresh_token&refresh_token={$arr['refresh_token']}";
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_URL, $url);
            ...
            $url
                = "https://api.weixin.qq.com/sns/userinfo?access_token={$arr['access_token']}&openid={$arr['openid']}&lang=zh_CN";
            $ch = curl_init();
            curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, false);
            curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
            curl_setopt($ch, CURLOPT_URL, $url);
            ...
            $userinfo = DB::name("user")
                ->field("id,user_nicename,avatar,avatar_thumb,sex,signature,coin,consumption,votestotal,province,city,birthday,user_status,login_type,last_login_time")
                ->where("openid!='' and openid='{$openid}' and login_type='{$type}'")
                ->find();
            $nowtime  = time();
            if (empty($userinfo)) {
                if ($openid == "") {
    ...
    public function getCode()
    {
        ...
        if (isset($_SESSION['mobile']) && $_SESSION['mobile'] == $mobile
            && isset($_SESSION['mobile_expiretime'])
            && $_SESSION['mobile_expiretime'] > time()
        ) {
            $rs['errno']  = 1007;
    ...
    public function userLogin()
    {
        ...
        $userinfo = Db::name("user")
            ->field("id,user_login,user_nicename,avatar,avatar_thumb,sex,signature,consumption,votestotal,province,city,coin,votes,birthday,issuper,user_status")
            ->where("user_login='{$user_login}' and user_type='2'")->find();
        ...
        $token      = md5(md5($userinfo['id'] . $userinfo['user_login']
            . time()));
        $expiretime = time() + 60 * 60 * 24 * 300;
    ...
    public function setNodeInfo()
    {
        ...
        if ($uid > 0) {
            ...
            $info['sign'] = $info['contribution'] . '.' . ($info['level'] + 100)
                . '1';
        } else {
            /* 游客 */
    ...
    public function getGift()
    {
        ...
        $gift = DB::name('gift')->field("id,type,giftname,needcoin,gifticon")
    ...
    public function sendGift()
    {
        ...
        $giftinfo = Db::name("gift")
            ->field("giftname,gifticon,needcoin,type,mark,swftype,swf,swftime")
            ->where("id='{$giftid}'")->find();
        if (!$giftinfo) {
        ...
        $where3['uid'] = $touid;
        $liveinfo      = Db::name("live")->where("islive=1")->where($where3)
            ->find();
        $showid        = 0;
        if ($liveinfo) {
        ...
        if ($giftinfo['type'] == 0 && $giftinfo['mark'] == 3) {
            ...
            $anthor_total = floor($anthor_total * $jackpotset['luck_anchor']
                * 0.01);
        }
        ...
        Db::name("user_coinrecord")->insert([
            "type"      => '0',
            "action"    => '1',
            "uid"       => $uid,
            "touid"     => $touid,
        ...
        /* 更新主播热门 */
        if ($giftinfo['mark'] == 1) {
            Db::name('live')->where(['uid', '=', $touid])->setInc('hotvotes',
                $total);
        }
        ...
        delCache("user:userinfo_" . $touid);


        $userinfo3 = Db::name('user')->field("votestotal")
            ->where("id='{$touid}'")->find();

        $gifttoken = md5(md5('sendGift' . $uid . $touid . $giftid . $giftcount
            . $total . $showid . $addtime));
        ...
        if ($ifluck == 1) {
            ...
            foreach ($rateinfo as $k => $v) {
                ...
                if ($luckrate <= $rate) {
                    ...
                    /* 用户加余额  写记录 */
                    Db::name('user')->where(['uid', '=', $uid])->setInc('coin',
                        $luckcoin);
        ...
        if ($ifup == 1) {
            ...
            if ($jackpotset['switch'] == 1 && $jackpotset['luck_jackpot'] > 0) {
                /* 开启奖池 */
                $jackpot_up = floor($total * $jackpotset['luck_jackpot']
                    * 0.01);

                //file_put_contents('./zhifu.txt',date('Y-m-d H:i:s').' 提交参数信息 jackpot_up:'.json_encode($jackpot_up)."\r\n",FILE_APPEND);
                if ($jackpot_up) {

                    Db::name('jackpot')->where(['id', '=', 1])->setInc('total',
                        $jackpot_up);
                ...
                if ($jackpotinfo['level'] >= 1) {
                    ...
                    if ($list) {
                        ...
                        if ($rateinfo) {
                            ...
                            $rate_jackpot
                                = json_decode($rateinfo['rate_jackpot'], true);

                            $rate = floor($rate_jackpot[$jackpotinfo['level']]
                                * 1000);
        ...
        $userinfo2 = Db::name('user')->field("consumption,coin,votestotal")
            ->where("id='{$uid}'")->find();
        ...
        echo '{"errno":"0","uid":"' . $uid . '","level":"' . $level
            . '","type":"' . $giftinfo['type'] . '","coin":"'
            . $userinfo2['coin'] . '","gifttoken":"' . $gifttoken
            . '","msg":"赠送成功"}';
        exit;
    ...
    public function pay()
    {
        ...
        $userinfo = Db::name("user")
            ->field("id,user_nicename,avatar_thumb,coin")->where("id='{$uid}'")
            ->find();
        $this->assign('userinfo', $userinfo);

        $chargelist = Db::name('charge_rules')
