...
class Portal extends TagLib
{
    ...
    protected $tags
        = [
            ...
            'articles'         => [
                'attr'  => 'field,where,limit,order,page,relation,returnVarName,pageVarName,categoryIds',
                'close' => 1,
            ],
            //非必须属性item
            'tagarticles'      => [
                'attr' => 'field,where,limit,order,page,relation,returnVarName,pageVarName,tagId',
                'close' => 1,
            ],
            //非必须属性item
            'breadcrumb'       => ['attr' => 'cid', 'close' => 1],
            //非必须属性self
            'categories'       => ['attr' => 'where,order', 'close' => 1],
            //非必须属性item
            'subcategories'    => ['attr' => 'categoryId', 'close' => 1],
            //非必须属性item
            'allsubcategories' => ['attr' => 'categoryId', 'close' => 1],
            //非必须属性item
        ];
    ...
    public function tagArticles($tag, $content)
    {
        ...
        $order         = empty($tag['order']) ? 'post.published_time DESC'
            : $tag['order'];
        $relation      = empty($tag['relation']) ? '' : $tag['relation'];
        $pageVarName   = empty($tag['pageVarName']) ? '__PAGE_VAR_NAME__'
            : $tag['pageVarName'];
        $returnVarName = empty($tag['returnVarName']) ? 'articles_data'
            : $tag['returnVarName'];
...
parse;
    ...
    public function tagTagArticles($tag, $content)
    {
        ...
        $pageVarName   = empty($tag['pageVarName']) ? '__PAGE_VAR_NAME__'
            : $tag['pageVarName'];
        $returnVarName = empty($tag['returnVarName']) ? 'tag_articles_data'
            : $tag['returnVarName'];
