...
class PortalPostModel extends Model
{
    ...
    {
        return $this->belongsToMany('PortalCategoryModel',
            'portal_category_post', 'category_id', 'post_id');
    }
    ...
    {
        return $this->belongsToMany('PortalTagModel', 'portal_tag_post',
            'tag_id', 'post_id');
    }
    ...
    public function adminAddArticle($data, $categories)
    {
        ...
        if (!empty($data['more']['audio'])) {
            $data['more']['audio']
                = cmf_asset_relative_url($data['more']['audio']);
        }

        if (!empty($data['more']['video'])) {
            $data['more']['video']
                = cmf_asset_relative_url($data['more']['video']);
        }
    ...
    public function addTags($keywords, $articleId)
    {
        ...
        if (!empty($keywords)) {
            ...
            $oldTagIds = Db::name('portal_tag_post')->where('post_id',
                $articleId)->column('tag_id');
            ...
            foreach ($keywords as $keyword) {
                ...
                if (!empty($keyword)) {
                    ...
                    if (!in_array($tagId, $oldTagIds)) {
                        array_push($data,
                            ['tag_id' => $tagId, 'post_id' => $articleId]);
                    }
            ...
            if (empty($tagIds) && !empty($oldTagIds)) {
                Db::name('portal_tag_post')->where('post_id', $articleId)
                    ->delete();
            }
    ...
    public function adminEditPage($data)
    {
        ...
        $routeModel->setRoute($data['post_alias'], 'portal/Page/index',
            ['id' => $data['id']], 2, 5000);
