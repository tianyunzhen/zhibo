...
class AdminArticleController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $categoryTree
                             = $portalCategoryModel->adminCategoryTree($categoryId);

        $this->assign('start_time',
            isset($param['start_time']) ? $param['start_time'] : '');
        $this->assign('end_time',
            isset($param['end_time']) ? $param['end_time'] : '');
        $this->assign('keyword',
            isset($param['keyword']) ? $param['keyword'] : '');
    ...
    public function add()
    {
        ...
        $articleThemeFiles
                    = $themeModel->getActionThemeFiles('portal/Article/index');
        ...
        if ($this->request->isPost()) {
            ...
            if (!empty($data['photo_names']) && !empty($data['photo_urls'])) {
                ...
                foreach ($data['photo_urls'] as $key => $url) {
                    ...
                    array_push($data['post']['more']['photos'], [
                        "url"  => $photoUrl,
                        "name" => $data['photo_names'][$key],
                    ]);
                }
            }
            ...
            if (!empty($data['file_names']) && !empty($data['file_urls'])) {
                ...
                foreach ($data['file_urls'] as $key => $url) {
                    ...
                    array_push($data['post']['more']['files'], [
                        "url"  => $fileUrl,
                        "name" => $data['file_names'][$key],
                    ]);
                }
            }
            ...
            $this->success('添加成功!',
                url('AdminArticle/edit', ['id' => $portalPostModel->id]));
        }
    ...
    public function edit()
    {
        ...
        $postCategories  = $post->categories()->alias('a')->column('a.name',
            'a.id');
    ...
    public function delete()
    {
        ...
        if (isset($param['id'])) {
            ...
            if ($resultPortal) {
                Db::name('portal_category_post')->where('post_id', $id)
                    ->update(['status' => 0]);
                Db::name('portal_tag_post')->where('post_id', $id)
                    ->update(['status' => 0]);
        ...
        if (isset($param['ids'])) {
            ...
            $result  = $portalPostModel->where('id', 'in', $ids)
                ->update(['delete_time' => time()]);
            if ($result) {
                Db::name('portal_category_post')->where('post_id', 'in', $ids)
                    ->update(['status' => 0]);
                Db::name('portal_tag_post')->where('post_id', 'in', $ids)
                    ->update(['status' => 0]);
    ...
    public function publish()
    {
        ...
        if (isset($param['ids']) && isset($param["yes"])) {
            ...
            $portalPostModel->where('id', 'in', $ids)
                ->update(['post_status' => 1, 'published_time' => time()]);
            $this->success("发布成功！", '');
        }
