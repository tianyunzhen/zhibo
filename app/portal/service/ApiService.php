...
class ApiService
{
    ...
    public static function articles($param)
    {
        ...
        $categoryIds = empty($param['category_ids']) ? ''
            : $param['category_ids'];
        ...
        if (!empty($categoryIds)) {
            ...
            $field = !empty($param['field']) ? $param['field']
                : 'post.*,min(category_post.category_id) as category_id';
            array_push($join, [
                '__PORTAL_CATEGORY_POST__ category_post',
                'post.id = category_post.post_id',
            ]);
            ...
            if (count($categoryIds) == 1) {
                ...
                $whereCategoryId = function (Query $query) use ($categoryIds) {
                    $query->where('category_post.category_id', 'in',
                        $categoryIds);
                };
            }
        } else {
    ...
    public static function tagArticles($param)
    {
        ...
        if (empty($tagId)) {
            ...
            array_push($join,
                ['__PORTAL_TAG_POST__ tag_post', 'post.id = tag_post.post_id']);
    ...
    public static function subCategories($categoryId, $field = '*')
    {
        ...
        return $portalCategoryModel->field($field)->where($where)
    ...
    public static function allSubCategories($categoryId)
    {
        ...
        if ($categoryId !== 0) {
            $category = $portalCategoryModel->field('path')->where('id',
                $categoryId)->find();
    ...
    public static function breadcrumb($categoryId, $withCurrent = false)
    {
        ...
        $path = $portalCategoryModel->where(['id' => $categoryId])
            ->value('path');
        ...
        if (!empty($path)) {
            ...
            if (!empty($parents)) {
                $data = $portalCategoryModel->where('id', 'in', $parents)
