...
class PostService
{
    ...
    public function adminPostList($filter, $isPage = false)
    {
        ...
        $category = empty($filter['category']) ? 0
            : intval($filter['category']);
        if (!empty($category)) {
            array_push($join, [
                '__PORTAL_CATEGORY_POST__ b',
                'a.id = b.post_id',
            ]);
            $field
                = 'a.*,b.id AS post_category_id,b.list_order,b.category_id,u.user_login,u.user_nicename,u.user_email';
        }
        ...
        $articles        = $portalPostModel->alias('a')->field($field)
            ...
            ->where(function (Query $query) use ($filter, $isPage) {
                ...
                $startTime = empty($filter['start_time']) ? 0
                    : strtotime($filter['start_time']);
                $endTime   = empty($filter['end_time']) ? 0
    ...
    public function publishedArticle($postId, $categoryId = 0)
    {
        ...
        if (empty($categoryId)) {
            ...
            $article = $portalPostModel->alias('post')->field('post.*')
                ->where($where)
                ->where('post.published_time', ['< time', time()],
                    ['> time', 0], 'and')
                ->find();
        } else {
            $where = [
            ...
            $join    = [
                [
                    '__PORTAL_CATEGORY_POST__ relation',
                    'post.id = relation.post_id',
                ],
            ];
            $article = $portalPostModel->alias('post')->field('post.*')
                ->join($join)
