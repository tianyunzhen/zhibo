...
class NavMenuController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $objResult = $navMenuModel->where("nav_id", $intNavId)
        ...
        foreach ($arrResult as $r) {
            $r['str_manage'] = '<a class="btn btn-xs btn-primary" href="'
                . url("NavMenu/add",
                    ["parent_id" => $r['id'], "nav_id" => $r['nav_id']]) . '">添加子菜单</a>
            <a class="btn btn-xs btn-primary" href="' . url("NavMenu/edit", [
                    "id"        => $r['id'],
                    "parent_id" => $r['parent_id'],
                    "nav_id"    => $r['nav_id'],
                ]) . '">编辑</a> 
            <a class="btn btn-xs btn-danger js-ajax-delete" href="'
                . url("NavMenu/delete",
                    ["id" => $r['id'], 'nav_id' => $r['nav_id']]) . '">删除</a> ';
    ...
    public function add()
    {
        ...
        foreach ($arrResult as $r) {
            $r['str_manage'] = '<a href="' . url("NavMenu/add",
                    ["parent_id" => $r['id']]) . '">添加子菜单</a> | <a href="'
                . url("NavMenu/edit", ["id" => $r['id']])
                . '">编辑</a> | <a class="J_ajax_del" href="'
    ...
    public function addPost()
    {
        ...
        if (isset($arrData['external_href'])) {
            $arrData['href']
                = htmlspecialchars_decode($arrData['external_href']);
        } else {
        ...
        $this->success(lang("EDIT_SUCCESS"),
            url("NavMenu/index", ['nav_id' => $arrData['nav_id']]));
    ...
    public function edit()
    {
        ...
        foreach ($arrResult as $r) {
            $r['str_manage'] = '<a href="' . url("NavMenu/add",
                    ["parent_id" => $r['id'], "nav_id" => $intNavId])
                . '">添加子菜单</a> | <a href="'
                . url("NavMenu/edit", ["id" => $r['id'], "nav_id" => $intNavId])
                . '">编辑</a> | <a class="js-ajax-delete" href="'
                . url("NavMenu/delete",
                    ["id" => $r['id'], "nav_id" => $intNavId]) . '">删除</a> ';
            $r['status']     = $r['status'] ? "显示" : "隐藏";
            $r['selected']   = $r['id'] == $intParentId ? "selected" : "";
            $array[]         = $r;
        }
    ...
    public function delete()
    {
        ...
        $this->success(lang("DELETE_SUCCESS"),
            url("NavMenu/index", ['nav_id' => $intNavId]));
