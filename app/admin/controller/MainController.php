...
class MainController extends AdminbaseController
{
    ...
    public function index()
    {
        ...
        $basic_today['newUsers']      = 0;
        $basic_today['totalUsers']    = 0;
        $basic_today['activityUsers'] = 0;
        $basic_today['launches']      = 0;
        if ($congifpri['um_appkey_android']) {
            $basic_today['newUsers']
                = number_format($basic_today_android['newUsers']
                + $basic_today_ios['newUsers']);
            $basic_today['totalUsers']
                = number_format($basic_today_android['totalUsers']
                + $basic_today_ios['totalUsers']);
            $basic_today['activityUsers']
                = number_format($basic_today_android['activityUsers']
                + $basic_today_ios['activityUsers']);
            $basic_today['launches']
                = number_format($basic_today_android['launches']
                + $basic_today_ios['launches']);
        }
        ...
        if ($source) {
            ...
            foreach ($source as $k => $v) {
                ...
                $data_source['nums_per'][] = round($v['nums'] * 100
                    / $nums_totoal);
            }
        }
        ...
        $anchor_live_long_total
            = Db::query("select sum(endtime - starttime) as times from cmf_live_record");
        ...
        /* 财务 */
        $charge_total = Db::name('charge_user')->where("status=1")
            ->sum("money");
        if (!$charge_total) {
            $charge_total = 0;
        }
    ...
    protected function getBasic($starttime, $endtime, $basic_type)
    {
        ...
        if ($congifpri['um_appkey_android']) {
            ...
            switch ($basic_type) {
                case '1':
                    $newusers_android = $this->getNewUsers($appkey, $start,
                        $end, $periodType);
                    break;
                case '2':
                    $launches_android = $this->getLaunches($appkey, $start,
                        $end, $periodType);
                    break;
                case '3':
                    $durations_android = $this->getDurations($appkey, $start,
                        $end, $periodType);
                    break;
                case '4':
                    $activeusers_android = $this->getActiveUsers($appkey,
                        $start, $end, $periodType);
                    break;
                case '5':
                    $retentions_android = $this->getRetentions($appkey, $start,
                        $end, $periodType);
                    break;
            }
        }
        ...
        if ($congifpri['um_appkey_ios']) {
            ...
            switch ($basic_type) {
                case '1':
                    $newusers_ios = $this->getNewUsers($appkey, $start, $end,
                        $periodType);
                    break;
                case '2':
                    $launches_ios = $this->getLaunches($appkey, $start, $end,
                        $periodType);
                    break;
                case '3':
                    $durations_ios = $this->getDurations($appkey, $start, $end,
                        $periodType);
                    break;
                case '4':
                    $activeusers_ios = $this->getActiveUsers($appkey, $start,
                        $end, $periodType);
                    break;
                case '5':
                    $retentions_ios = $this->getRetentions($appkey, $start,
                        $end, $periodType);
                    break;
            }
        }
        ...
        switch ($basic_type) {
            case '1':
                if ($periodType == 'hourly') {
                    ...
                    foreach ($rs['name'] as $k => $v) {
                        ...
                        $newUsers_a
                                    = isset($newusers_android[0]['hourValue'][$k])
                            ? $newusers_android[0]['hourValue'][$k] : 0;
                        $newUsers_i = isset($newusers_ios[0]['hourValue'][$k])
                            ? $newusers_ios[0]['hourValue'][$k] : 0;
                    ...
                    /* value */
                    foreach ($rs['name'] as $k => $v) {
                        $newUsers_a = isset($newusers_android[$k]['value'])
                            ? $newusers_android[$k]['value'] : 0;
                        $newUsers_i = isset($newusers_ios[$k]['value'])
                            ? $newusers_ios[$k]['value'] : 0;
            ...
            case '2':
                if ($periodType == 'hourly') {
                    ...
                    foreach ($rs['name'] as $k => $v) {
                        $launches_a
                                    = isset($launches_android[0]['hourValue'][$k])
                            ? $launches_android[0]['hourValue'][$k] : 0;
                        $launches_i = isset($launches_ios[0]['hourValue'][$k])
                            ? $launches_ios[0]['hourValue'][$k] : 0;
                    ...
                    foreach ($rs['name'] as $k => $v) {
                        ...
                        $launches_a = isset($launches_android[$k]['value'])
                            ? $launches_android[$k]['value'] : 0;
                        $launches_i = isset($launches_ios[$k]['value'])
                            ? $launches_ios[$k]['value'] : 0;
            ...
            case '3':
                ...
                foreach ($rs['name'] as $k => $v) {
                    $durations_a = isset($durations_android[$k])
                        ? $durations_android[$k] : 0;
                    $durations_i = isset($durations_ios[$k])
                        ? $durations_ios[$k] : 0;
            ...
            case '4':
                if ($periodType == 'hourly') {
                    ...
                    foreach ($rs['name'] as $k => $v) {
                        ...
                        $activityUsers_a
                            = isset($activeusers_android[0]['hourValue'][$k])
                            ? $activeusers_android[0]['hourValue'][$k] : 0;
                        $activityUsers_i
                            = isset($activeusers_ios[0]['hourValue'][$k])
                            ? $activeusers_ios[0]['hourValue'][$k] : 0;
                    ...
                    foreach ($rs['name'] as $k => $v) {
                        ...
                        $activityUsers_a
                                         = isset($activeusers_android[$k]['value'])
                            ? $activeusers_android[$k]['value'] : 0;
                        $activityUsers_i = isset($activeusers_ios[$k]['value'])
                            ? $activeusers_ios[$k]['value'] : 0;
            ...
            case '5':
                if ($periodType == 'hourly') {
                    ...
                    foreach ($rs['name'] as $k => $v) {
                        ...
                        $retentions_a
                            = isset($retentions_android[0]['hourValue'][$k])
                            ? $retentions_android[0]['hourValue'][$k] : 0;
                        $retentions_i
                            = isset($retentions_ios[0]['hourValue'][$k])
                            ? $retentions_ios[0]['hourValue'][$k] : 0;
                    ...
                    foreach ($rs['name'] as $k => $v) {
                        ...
                        $retentions_a = isset($retentions_android[$k]['value'])
                            ? $retentions_android[$k]['value'] : 0;
                        $retentions_i = isset($retentions_ios[$k]['value'])
                            ? $retentions_ios[$k]['value'] : 0;
        ...
        if ($basic_type == 3) {
            $rs['nums'] = floor(array_sum($rs['data']) / (($endtime
                        - $starttime) / (60 * 60 * 24)));
        } else {
    ...
    protected function getUmengData($urlPath, $data)
    {
        ...
        $s         = $urlPath . $param;
        $Signature = strtoupper(bin2hex(hash_hmac("sha1", $s, $apiSecurity,
            true)));
    ...
    {
        $anchor_today = Db::name('live_record')
            ->where("starttime >= {$start} and starttime<{$end}")->group("uid")
            ->count();
        if (!$anchor_today) {
            $anchor_today = 0;
        }
        $anchor_live_today = Db::name('live_record')
            ->where("starttime >= {$start} and starttime<{$end}")->count();

        $anchor_live_long_today
               = Db::query("select sum(endtime - starttime) as times from cmf_live_record where starttime >= {$start} and starttime<{$end}");
        $times = 0;
    ...
    protected function getCharge($start, $end)
    {
        ...
        $charge_total_today = Db::name('charge_user')
            ->where("status=1 and addtime>={$start} and addtime<{$end}")
            ->sum("money");
        ...
        foreach ($charge_type as $k => $v) {
            $data_charge['name'][] = $v;
            $money                 = Db::name('charge_user')
                ->where("status=1 and type={$k} and addtime>={$start} and addtime<{$end}")
                ->sum("money");
            if (!$money) {
                $money = 0;
            }
    ...
    {
        $cash_apply = Db::name('cash_record')
            ->where("status=0 and addtime>={$start} and addtime<{$end}")
            ->sum("money");
        if (!$cash_apply) {
            $cash_apply = 0;
        }
        ...
        $cash_adopt = Db::name('cash_record')
            ->where("status=1 and addtime>={$start} and addtime<{$end}")
            ->sum("money");
        if (!$cash_adopt) {
            $cash_adopt = 0;
        ...
        $cash_anchor = Db::name('cash_record')
            ->where("addtime>={$start} and addtime<{$end}")->group("uid")
            ->count();
        if (!$cash_anchor) {
            $cash_anchor = 0;
        }
    ...
    function export()
    {
        ...
        switch ($action) {
            case '1':
                ...
                if ($congifpri['um_appkey_android']) {
                    ...
                    $launches_android = $this->getLaunches($appkey, $start,
                        $end, $periodType);
                    if ($periodType != 'hourly') {
                        $durations_android = $this->getDurations($appkey,
                            $start, $end, $periodType);
                    }
                ...
                if ($congifpri['um_appkey_ios']) {
                    ...
                    $launches_ios = $this->getLaunches($appkey, $start, $end,
                        $periodType);
                    if ($periodType != 'hourly') {
                        $durations_ios = $this->getDurations($appkey, $start,
                            $end, $periodType);
                    }
                    $activeusers_ios = $this->getActiveUsers($appkey, $start,
                        $end, $periodType);
                ...
                if ($periodType != 'hourly') {
                    for (
                        $i = $starttime, $n = 0; $i < $endtime;
                        $i += 60 * 60 * 24, $n++
                    ) {
                        $newUsers_a = isset($newusers_android[$n]['value'])
                            ? $newusers_android[$n]['value'] : 0;
                        $newUsers_i = isset($newusers_ios[$n]['value'])
                            ? $newusers_ios[$n]['value'] : 0;

                        $launches_a = isset($launches_android[$n]['value'])
                            ? $launches_android[$n]['value'] : 0;
                        $launches_i = isset($launches_ios[$n]['value'])
                            ? $launches_ios[$n]['value'] : 0;

                        $duration_a = isset($durations_android[$n])
                            ? $durations_android[$n] : 0;
                        $duration_i = isset($durations_ios[$n])
                            ? $durations_ios[$n] : 0;

                        $activityUsers_a
                                         = isset($activeusers_android[$n]['value'])
                            ? $activeusers_android[$n]['value'] : 0;
                        $activityUsers_i = isset($activeusers_ios[$n]['value'])
                            ? $activeusers_ios[$n]['value'] : 0;
                        ...
                        $info['duration']      = floor(($duration_a
                                + $duration_i) / 60);
                        $info['activityUsers'] = $activityUsers_a
                            + $activityUsers_i;
                        $info['ismerge']       = '0';
                        $xlsData[]             = $info;
                    }
                    ...
                    for ($n = 0; $n < 24; $n++) {
                        ...
                        $newUsers_a
                                    = isset($newusers_android[0]['hourValue'][$n])
                            ? $newusers_android[0]['hourValue'][$n] : 0;
                        $newUsers_i = isset($newusers_ios[0]['hourValue'][$n])
                            ? $newusers_ios[0]['hourValue'][$n] : 0;

                        $launches_a
                                    = isset($launches_android[0]['hourValue'][$n])
                            ? $launches_android[0]['hourValue'][$n] : 0;
                        $launches_i = isset($launches_ios[0]['hourValue'][$n])
                            ? $launches_ios[0]['hourValue'][$n] : 0;


                        $activityUsers_a
                            = isset($activeusers_android[0]['hourValue'][$n])
                            ? $activeusers_android[0]['hourValue'][$n] : 0;
                        $activityUsers_i
                            = isset($activeusers_ios[0]['hourValue'][$n])
                            ? $activeusers_ios[0]['hourValue'][$n] : 0;
            ...
            case '4':
                ...
                $xlsCell = ['date', 'total', 'ali', 'wx', 'apple'];
                ...
                $xlsData[] = [
                    'date'    => date("Y-m-d", $start) . '至' . date("Y-m-d",
                            ($end - 1)),
                    'total'   => '',
                    'ali'     => '',
                    'wx'      => '',
            ...
            case '5':
                ...
                $cash_total = Db::name('cash_record')->where("status=1")
                    ->sum("money");
                if (!$cash_total) {
                    $cash_total = 0;
                }
                ...
                $xlsCell = ['date', 'cash_apply', 'cash_adopt', 'cash_anchor'];
    ...
    protected function exportExcel(
        ...
        $fileName = $xlsTitle . '_'
            . date('YmdHis');//or $xlsTitle 文件名称可根据自己情况设定
        ...
        for ($i = 0; $i < $dataNum; $i++) {
            ...
            if ($cellinfo['ismerge'] == 1) {
                $objPHPExcel->getActiveSheet()->mergeCells('A' . ($i + 1) . ':'
                    . end($cellName) . ($i
                        + 1));//合并单元格（如果要拆分单元格是需要先合并再拆分的，否则程序会报错）

                $objPHPExcel->getActiveSheet(0)->setCellValue('A' . ($i + 1),
                    $cellinfo[$expCellName[0]]);
            } else {
                for ($j = 0; $j < $cellNum; $j++) {
                    $key = $expCellName[$j];
