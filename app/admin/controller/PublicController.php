...
//        }
        ...
        if (!empty($result) && $result['user_type'] == 1) {
            //dump(cmf_password($pass)); dump($result['user_pass']);exit;
            if (cmf_compare_password($pass, $result['user_pass'])) {
                $groups = Db::name('RoleUser')
                    ...
                    ->value("role_id");
                if (($result["id"] != 1 && $result["id"] != 10000000)
                    && (empty($groups) || empty($result['user_status']))
                ) {
                ...
                $token
                    = cmf_generate_user_token($result["id"], 'web');
                if (!empty($token)) {
                    session('token', $token);
                }
