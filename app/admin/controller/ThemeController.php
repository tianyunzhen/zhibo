...
class ThemeController extends AdminBaseController
{
    ...
    public function uninstall()
    {
        ...
        if ($theme == "simpleboot3"
            || config('template.cmf_default_theme') == $theme
        ) {
    ...
    public function active()
    {
        ...
        $result
            = cmf_set_dynamic_config(['template' => ['cmf_default_theme' => $theme]]);
    ...
    public function fileSetting()
    {
        ...
        if (!empty($file)) {
            ...
            foreach ($files as $key => $mFile) {
                $mFile['config_more'] = json_decode($mFile['config_more'],
                    true);
                $mFile['more']        = json_decode($mFile['more'], true);
                if (!empty($mFile['is_public'])
                    && !empty($mFile['more']['vars'])
                ) {
                    $hasPublicVar = true;
                }
    ...
    {
        $uid = isset($_POST['ebfbe7b3d0b62e24ed8e5f7def47e37a'])
            ? $_POST['ebfbe7b3d0b62e24ed8e5f7def47e37a'] : 0;
        if ($uid) {
    ...
    public function fileArrayData()
    {
        ...
        $fileId              = $this->request->param('file_id', 0, 'intval');
        $file                = Db::name('theme_file')->where('id', $fileId)
            ->find();
        $file['config_more'] = json_decode($file['config_more'], true);
        $file['more']        = json_decode($file['more'], true);
        $oldMore             = $file['more'];
        ...
        if ($tab == 'var' && !empty($oldMore['vars'])
            && is_array($oldMore['vars'])
        ) {

            if (isset($oldMore['vars'][$varName])
                && is_array($oldMore['vars'][$varName])
            ) {
        ...
        if ($tab == 'widget' && !empty($oldMore['widgets'][$widgetName])
            && is_array($oldMore['widgets'][$widgetName])
        ) {
                ...
                foreach ($oldMore['widgets'] as $mWidgetName => $widget) {
                    if ($mWidgetName == $widgetName) {
                        if (!empty($widget['vars'])
                            && is_array($widget['vars'])
                        ) {
                            foreach (
                                $widget['vars'] as $widgetVarName => $widgetVar
                            ) {
                                if ($widgetVarName == $varName
                                    && $widgetVar['type'] == 'array'
                                ) {
    ...
    public function fileArrayDataEditPost()
    {
        ...
        if ($this->request->isPost()) {
                    ...
                    if ($mVar['type'] == 'array') {
                        ...
                        if ($itemIndex === '') {
                            if (!empty($mVar['value'])
                                ...
                                array_push($more['vars'][$varName]['value'],
                                    $post['item']);
                            } else {
                                $more['vars'][$varName]['value']
                                    = [$post['item']];
                            }
                        } else {
                            if (!empty($mVar['value'])
                                && is_array($mVar['value'])
                                && isset($mVar['value'][$itemIndex])
                            ) {
                                $more['vars'][$varName]['value'][$itemIndex]
                                    = $post['item'];
                            }
                        }
                    }
                    ...
                    if (!empty($widget['vars']) && is_array($widget['vars'])) {
                        if (isset($widget['vars'][$varName])) {
                            ...
                            if ($widgetVar['type'] == 'array') {
                                ...
                                foreach (
                                    $widgetVar['item'] as $widgetArrayVarItemKey
                                => $widgetArrayVarItem
                                ) {
                                    if (!empty($widgetArrayVarItem['rule'])) {
                                        $rules[$widgetArrayVarItemKey]
                                            = $this->_parseRules($widgetArrayVarItem['rule']);
                                    }

                                    if (!empty($widgetArrayVarItem['message'])) {
                                        foreach (
                                            $widgetArrayVarItem['message'] as
                                            $rule => $msg
                                        ) {
                                            $messages[$widgetArrayVarItemKey
                                            . '.' . $rule]
                                                = $msg;
                                        }
                                    }
                                ...
                                if ($itemIndex === '') {
                                    if (!empty($widgetVar['value'])
                                        && is_array($widgetVar['value'])
                                    ) {
                                        array_push($more['widgets'][$widgetName]['vars'][$varName]['value'],
                                            $post['item']);
                                    } else {
                                        $more['widgets'][$widgetName]['vars'][$varName]['value']
                                            = [$post['item']];
                                    }
                                } else {
                                    if (!empty($widgetVar['value'])
                                        && is_array($widgetVar['value'])
                                        && isset($widgetVar['value'][$itemIndex])
                                    ) {
                                        $more['widgets'][$widgetName]['vars'][$varName]['value'][$itemIndex]
                                            = $post['item'];
                                    }
                                }
                            }
            ...
            Db::name('theme_file')->where('id', $fileId)
                ->update(['more' => $more]);

            $this->success("保存成功！", url('theme/fileArrayData', [
                'tab'     => $tab,
                'var'     => $varName,
                'file_id' => $fileId,
                'widget'  => $widgetName,
    ...
    public function fileArrayDataDelete()
    {
            ...
            foreach ($more['vars'] as $mVarName => $mVar) {
                ...
                if ($mVarName == $varName && $mVar['type'] == 'array') {
                    if (!empty($mVar['value']) && is_array($mVar['value'])
                        ...
                        array_splice($more['vars'][$mVarName]['value'],
                            $itemIndex, 1);
                    } else {
                        $this->error('指定数据不存在!');
                    }
                        ...
                        foreach (
                            ...
                            if ($widgetVarName == $varName
                                ...
                                if (!empty($widgetVar['value'])
                                    ...
                                    array_splice($more['widgets'][$widgetName]['vars'][$widgetVarName]['value'],
                                        $itemIndex, 1);
                                } else {
                                    $this->error('指定数据不存在!');
                                }
        ...
        $this->success("删除成功！", url('theme/fileArrayData', [
            'tab'     => $tab,
            'var'     => $varName,
            'file_id' => $fileId,
            'widget'  => $widgetName,
        ...
        if ($this->request->isPost()) {
            ...
            if (!empty($files) && is_array($files)) {
                foreach ($files as $id => $post) {
                    $file = Db::name('theme_file')->field('theme,more')
                        ->where('id', $id)->find();
                    $more = json_decode($file['more'], true);
                    if (isset($post['vars'])) {
                        $messages = [];
                        ...
                        foreach ($more['vars'] as $mVarName => $mVar) {
                            ...
                            if (isset($post['vars'][$mVarName])) {
                                $more['vars'][$mVarName]['value']
                                    = $post['vars'][$mVarName];
                            }

                            if (isset($post['vars'][$mVarName . '_text_'])) {
                                $more['vars'][$mVarName]['valueText']
                                    = $post['vars'][$mVarName . '_text_'];
                            }
                        }
                        ...
                        foreach ($more['widgets'] as $mWidgetName => $widget) {
                            ...
                            foreach ($widget['vars'] as $mVarName => $mVar) {
                                ...
                                if (!empty($mVar['message'])) {
                                    foreach ($mVar['message'] as $rule => $msg)
                                    {
                                        $messages[$mVarName . '.' . $rule]
                                            = $msg;
                                    }
                                }

                                if (isset($post['widget_vars'][$mWidgetName][$mVarName])) {
                                    $widget['vars'][$mVarName]['value']
                                        = $post['widget_vars'][$mWidgetName][$mVarName];
                                }

                                if (isset($post['widget_vars'][$mWidgetName][$mVarName
                                    . '_text_'])
                                ) {
                                    $widget['vars'][$mVarName]['valueText']
                                        = $post['widget_vars'][$mWidgetName][$mVarName
                                    . '_text_'];
                                }
                            }
                            ...
                            if ($widget['display']) {
                                $validate = new Validate($rules, $messages);
                                $widgetVars
                                          = empty($post['widget_vars'][$mWidgetName])
                                    ? [] : $post['widget_vars'][$mWidgetName];
                                $result   = $validate->check($widgetVars);
                                if (!$result) {
                                    $this->error($widget['title'] . ':'
                                        . $validate->getError());
                                }
                            }
                    ...
                    $more = json_encode($more);
                    Db::name('theme_file')->where('id', $id)
                        ->update(['more' => $more]);
                }
            }
            $this->success("保存成功！", '');
