...
class SettingController extends AdminBaseController
{
    ...
    public function site()
    {
        ...
        $adminThemesDir = WEB_ROOT . config('template.cmf_admin_theme_path')
            . config('template.cmf_admin_default_theme')
        ...
        $adminThemes = [];
        $themes      = cmf_scan_dir(WEB_ROOT
            . config('template.cmf_admin_theme_path') . '/*', GLOB_ONLYDIR);
    ...
    public function sitePost()
    {
        if ($this->request->isPost()) {
            ...
            $bannedUsernames
                                             = preg_replace("/[^0-9A-Za-z_\\x{4e00}-\\x{9fa5}-]/u",
                ",", $cmfSettings['banned_usernames']);
            ...
            if (!empty($adminSettings['admin_password'])) {
                $routeModel->setRoute($adminSettings['admin_password'] . '$',
                    'admin/Index/index', [], 2, 5000);
            } else {
            ...
            delcache('common:config:pub_confg');
            $this->success("保存成功！", '');
        }
    }
                ...
                if ($password == $rePassword) {
                    ...
                    if (cmf_compare_password($password, $admin['user_pass'])) {
                        ...
                        Db::name('user')->where('id', $userId)
                            ->update(['user_pass' => cmf_password($password)]);
                        $this->success("密码修改成功！");
                    }
                } else {
                    $this->error("密码输入不一致！");
