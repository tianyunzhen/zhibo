...
class SlideItemController extends AdminBaseController
{
    ...
    public function addPost()
    {
        ...
        $this->success("添加成功！",
            url("slideItem/index", ['slide_id' => $data['post']['slide_id']]));
    }
...
//            }
            $this->success("删除成功！",
                url("SlideItem/index", ["slide_id" => $slideItem['slide_id']]));
        } else {
            $this->error('删除失败！');
        }
    ...
    public function ban()
    {
        $id = $this->request->param('id', 0, 'intval');
        if ($id) {
            $rst = Db::name('slideItem')->where('id', $id)
                ->update(['status' => 0]);
            if ($rst) {
                $this->resetcache();
    ...
    public function cancelBan()
    {
        $id = $this->request->param('id', 0, 'intval');
        if ($id) {
            $result = Db::name('slideItem')->where('id', $id)
                ->update(['status' => 1]);
            if ($result) {
                $this->resetcache();
    ...
    {
        $key = 'common:getSlide';
        $rs  = Db::name("slideItem")
            ->field("image as slide_pic,url as slide_url")
            ->where("status='1' and slide_id='2' ")->order("list_order asc")
            ->select()->toArray();
        if ($rs) {
            foreach ($rs as $k => $v) {
