...
class PluginController extends AdminBaseController
{
    ...
    public function toggle()
    {
        ...
        try {
            ...
            $hookPluginModel->save(['status' => $status],
                ['plugin' => $pluginModel->name]);
                ...
                if ($value['type'] != 'group') {
                    if (isset($pluginConfigInDb[$key])) {
                        $plugin['config'][$key]['value']
                            = $pluginConfigInDb[$key];
                    }
                } else {
                    foreach ($value['options'] as $group => $options) {
                        foreach ($options['options'] as $gkey => $value) {
                            if (isset($pluginConfigInDb[$gkey])) {
                                $plugin['config'][$key]['options'][$group]['options'][$gkey]['value']
                                    = $pluginConfigInDb[$gkey];
                            }
                        }
                    }
                }
...
//        }
    ...
    public function update()
    {
        ...
        $pluginHooksInDb = $hookPluginModel->where('plugin', $pluginName)
            ->column('hook');
    ...
    private function _getActions($pluginName)
    {
        ...
        $annotationManager
                                      = Annotations::getManager();
        $annotationManager->registry['adminMenu']
                                      = 'app\admin\annotation\AdminMenuAnnotation';
        $annotationManager->registry['adminMenuRoot']
                                      = 'app\admin\annotation\AdminMenuRootAnnotation';
        $newMenus                     = [];

        $pluginDir = cmf_parse_name($pluginName);

        $filePatten = CMF_ROOT . 'plugins/' . $pluginDir
        ...
        if (!empty($controllers)) {
            foreach ($controllers as $controller) {
                ...
                $controllerName = preg_replace('/\Controller$/', '',
                    $controller);
                $controllerClass
                                = "plugins\\$pluginDir\\controller\\$controller";
...
//                            array_push($newMenus, $app."/$controllerName/$action 层级关系已更新");
                        ...
                        $authRuleName
                                           = "plugin/{$pluginName}/{$controllerName}/{$action}";
                ...
                if (!empty($methods)) {
                    foreach ($methods as $method) {
                        ...
                        if ($method->class == $controllerClass
                            && strpos($method->name, '_') !== 0
                        ) {
                            $menuAnnotations
                                = Annotations::ofMethod($controllerClass,
