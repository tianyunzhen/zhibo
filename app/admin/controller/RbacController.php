...
class RbacController extends AdminBaseController
{
    ...
    public function authorize()
    {
        ...
        $newMenus      = [];
        $privilegeData = $AuthAccess->where("role_id", $roleId)
            ->column("rule_name");//获取权限表数据
        ...
        foreach ($result as $n => $t) {
            $result[$n]['checked']      = ($this->_isChecked($t,
                $privilegeData)) ? ' checked' : '';
            $result[$n]['level']        = $this->_getLevel($t['id'], $newMenus);
            $result[$n]['style']        = empty($t['parent_id']) ? ''
                : 'display:none;';
            $result[$n]['parentIdNode'] = ($t['parent_id'])
                ? ' class="child-of-node-' . $t['parent_id'] . '"' : '';
        }
    ...
    public function authorizePost()
    {
        if ($this->request->isPost()) {
            ...
            if (is_array($this->request->param('menuId/a'))
                && count($this->request->param('menuId/a')) > 0
            ) {

                Db::name("authAccess")->where([
                    "role_id" => $roleId,
                    'type'    => 'admin_url',
                ])->delete();
    ...
    protected function _getLevel($id, $array = [], $i = 0)
    {
        if ($array[$id]['parent_id'] == 0
            || empty($array[$array[$id]['parent_id']])
