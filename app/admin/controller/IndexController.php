...
class IndexController extends AdminBaseController
{
    ...
    public function initialize()
    {
        ...
        if (empty($adminSettings['admin_password'])
            || $this->request->path() == $adminSettings['admin_password']
        ) {
...
//        $menus          = cache('admin_menus_' . cmf_get_current_admin_id(), '',
//            null, 'admin_menus');

            $menus = $adminMenuModel->menuTree();
//            var_dump($menus);die;
//            cache('admin_menus_' . cmf_get_current_admin_id(), $menus, null,
//                'admin_menus');
        ...
        foreach ($result as $item) {
            //去掉/ _ 全部小写。作为索引。
            $indexTmp            = $item['app'] . $item['controller']
                . $item['action'];
