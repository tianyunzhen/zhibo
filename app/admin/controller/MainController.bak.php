...
class MainController extends AdminBaseController
{
    ...
    public function index()
    {
        ...
        $defaultDashboardWidgets = [
            ...
            '_SystemMainContributors' => [
                'name'      => 'MainContributors',
                'is_system' => 1,
            ],
            '_SystemContributors'     => [
                'name'      => 'Contributors',
        ...
        if (empty($widgets)) {
            ...
            foreach ($widgets as $widget) {
                if ($widget['is_system']) {
                    $dashboardWidgets['_System' . $widget['name']]
                        = ['name' => $widget['name'], 'is_system' => 1];
                } else {
                    $dashboardWidgets[$widget['name']]
                        = ['name' => $widget['name'], 'is_system' => 0];
                }
            }
        ...
        if (!empty($hookResults)) {
            foreach ($hookResults as $hookResult) {
                if (isset($hookResult['width']) && isset($hookResult['view'])
                    && isset($hookResult['plugin'])
                ) { //验证插件返回合法性
                    $dashboardWidgetPlugins[$hookResult['plugin']]
                        = $hookResult;
                    if (!isset($dashboardWidgets[$hookResult['plugin']])) {
                        $dashboardWidgets[$hookResult['plugin']]
                            = [
                            'name'      => $hookResult['plugin'],
                            'is_system' => 0,
                        ];
                    }
                }
