...
class Model_Video extends PhalApi_Model_NotORM
{
    ...
    public function setVideo($data, $music_id)
    {
        ...
        //视频分类是否存在
        if ($data['classid']) {
            $isexitclass = DI()->notorm->video_class->where("id=?",
                $data['classid'])->fetchOne();
            if (!$isexitclass) {
                return 1007;//视频分类不存在
            }
        }
        ...
        if ($music_id > 0) { //更新背景音乐被使用次数
            DI()->notorm->music
                ->where("id = '{$music_id}'")
                ->update(['use_nums' => new NotORM_Literal("use_nums + 1")]);
        }
    ...
    public function addView($uid, $videoid)
    {
        ...
        DI()->notorm->video
            ->where("id = '{$videoid}'")
            ->update(['views' => new NotORM_Literal("views + 1")]);
    ...
    public function addStep($uid, $videoid)
    {
        ...
        if ($like) {
            ...
            DI()->notorm->video
                ...
                ->update(['steps' => new NotORM_Literal("steps - 1")]);
            $rs['isstep'] = '0';
        } else {
            DI()->notorm->video_step
                ->insert([
    ...
    public function addShare($uid, $videoid)
    {
        ...
        DI()->notorm->video
            ->where("id = '{$videoid}'")
            ->update(['shares' => new NotORM_Literal("shares + 1")]);
        $rs['isshare'] = '1';
    ...
    public function addCommentLike($uid, $commentid)
    {
        ...
        //获取视频信息
        $videoinfo = DI()->notorm->video->select("thumb")
            ->where("id='{$commentinfo['videoid']}'")->fetchOne();
    ...
    public function getAttentionVideo($uid, $p)
    {
        ...
        if ($attention) {
            ...
            $where
                        = "uid in ({$touids}) and id not in ({$videoids_s})  and isdel=0 and status=1";
    ...
    public function getComments($uid, $videoid, $p)
    {
        ...
        foreach ($comments as $k => $v) {
            ...
            if ($uid) {
                $comments[$k]['islike'] = (string)$this->ifCommentLike($uid,
                    $v['id']);
            } else {
                $comments[$k]['islike'] = '0';
            }
    ...
    public function getHomeVideo($uid, $touid, $p)
    {
        ...
        if ($uid == $touid) {  //自己的视频（需要返回视频的状态前台显示）
            ...
            $videoids_s = getVideoBlack($uid);
            $where
                        = "id not in ({$videoids_s}) and uid={$touid} and isdel='0' and status=1  and is_ad=0";
        }
    ...
    public function getRecommendVideos($uid, $p, $isstart)
    {
        ...
        if ($video_showtype == 0) { //随机
            ...
            $info = DI()->notorm->video
                ->select("*,(ceil(comments * " . $comment_weight . " + likes * "
                    . $like_weight . " + shares * " . $share_weight
                    . "))* if(format(watch_ok/views,2) >1,'1',format(watch_ok/views,2)) as recomend")
                ->where("isdel=0 and status=1 and is_ad=0")
                // ->where('not id',$where)
    ...
    public function getNearby($uid, $lng, $lat, $p)
    {
        ...
        $info
            = DI()->notorm->video->queryAll("select *, round(6378.138 * 2 * ASIN(SQRT(POW(SIN(( "
            . $lat . " * PI() / 180 - lat * PI() / 180) / 2),2) + COS(" . $lat
            . " * PI() / 180) * COS(lat * PI() / 180) * POW(SIN((" . $lng
            . " * PI() / 180 - lng * PI() / 180) / 2),2))) * 1000) AS distance FROM "
            . $prefix . "video  where uid !=" . $uid
            . " and isdel=0 and status=1  and is_ad=0 order by distance asc,addtime desc limit "
            . $start . "," . $pnum);
    ...
    public function setConversion($videoid)
    {
        ...
        $res = DI()->notorm->video
            ...
            ->update(['watch_ok' => new NotORM_Literal("watch_ok + 1")]);
