...
class Model_Music extends PhalApi_Model_NotORM
{
    ...
    {
        $list = DI()->notorm->music_classify->select("id,title,img_url")
            ->where("isdel=0")->order("list_order asc")->fetchAll();
        if (!$list) {
            return 1001;
        }
    ...
    public function music_list($classify, $uid, $p)
    {
        ...
        $list
            = DI()->notorm->music->select("id,title,author,img_url,length,file_url,use_nums")
            ->where($where)->order("use_nums desc")->limit($start, $nums)
            ->fetchAll();
        ...
        if ($uid < 1) { //游客
            ...
            //获取本人收藏列表
            $collectLists = DI()->notorm->music_collection->select("music_id")
                ->where("uid=? and status=1", $uid)->fetchAll();
            $collects     = [];
    ...
    public function collectMusic($uid, $musicid)
    {
        ...
        //判断音乐是否存在
        $info = DI()->notorm->music->select("title,addtime")->where("id=?",
            $musicid)->fetchOne();
        ...
        //判断用户是否收藏过该视频
        $isexist = DI()->notorm->music_collection->select("*")
            ->where("uid='{$uid}' and music_id='{$musicid}'")->fetchOne();
        ...
        if ($isexist) {
            ...
            if ($isexist['status'] == 1) { //已收藏
                //将状态改为取消收藏
                $result
                    = DI()->notorm->music_collection->where("uid=? and music_id=?",
                    $uid, $musicid)->update([
                    "status"     => 0,
                    "updatetime" => time(),
                ]);
    ...
    public function getCollectMusicLists($uid, $p)
    {
        ...
        $list = DI()->notorm->music_collection->select("music_id,addtime")
            ->where($where)->order("addtime desc")->limit($start, $nums)
            ->fetchAll();
        ...
        foreach ($list as $k => $v) {
            $musicinfo
                = DI()->notorm->music->select("title,author,img_url,length,file_url,use_nums")
                ->where("id=?", $v['music_id'])->fetchOne();
            ...
            $list[$k]['iscollect']   = '1';
            $list[$k]['id']
                                     = $v['music_id'];  //为了app端模板解析统一，所以重新添加了一个id字段
