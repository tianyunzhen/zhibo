...
class Model_Cdnrecord extends PhalApi_Model_NotORM
{
    ...
    public function getCdnRecord($id)
    {
        ...
        $result
            = DI()->notorm->live_record->select('id,starttime,endtime,stream,video_url')
    ...
    protected function alicdn($data)
    {
        ...
        $video_url
            = $res_arr['RecordIndexInfoList']['RecordIndexInfo'][0]['RecordUrl'];

        DI()->notorm->live_record->where('id=?', $data['id'])
    ...
    protected function getStringToSign($parameter, $access_key_secret)
    {
        ...
        $url = "https://live.aliyuncs.com/?" . $ss . "&Signature="
